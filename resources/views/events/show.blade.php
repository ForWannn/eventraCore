    ...
    @if($isAssigned && !$myAttendance && $event->status === 'ongoing' && $attendanceOpen)
        <script>
            ...
            function getLocation() {
                ...
                navigator.geolocation.getCurrentPosition(
                    async (position) => {
                        ...
                        // Enable capture button & update UI
                        locStatus.style.color = "#166534"; // hijau sukses
            ...
            function takePhoto() {
                ...
                // Normalnya webcame dibalik (mirrored) saat preview.
                // Tapi untuk foto geotag, lebih baik TIDAK MIRROR agar font gak terbalik kalau kita tempel.
                // Oleh karena itu, kita balik DULU gambar videonya, barulah kita tulis teks normal.
                ctx.translate(canvas.width, 0);
                ctx.scale(-1, 1);
                ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
                
                // KEMBALIKAN ALUR CANVAS ke Normal (gak terbalik) supaya text bisa dibaca
                ctx.translate(canvas.width, 0);
                ctx.scale(-1, 1);

                // Menggambar Watermark Hitam Semi Transparan di Bawah
                const rectHeight = 150; // Diperbesar untuk menampung alamat full
                ...
                // 1. Gambar Nama Aplikas/Status
                ctx.font = 'bold 16px sans-serif';
                ctx.fillText('{{ config("app.name") }} Geotag Absensi', paddingX, startY);

                // 2. Gambar Tanggal dan Jam
                ctx.font = 'bold 14px monospace';
                ctx.fillStyle = '#fef08a'; // kuning terang (mirip cap kamera kuno/geotag)
                const dateTimeStr = formatLocalDate();
                ctx.fillText(dateTimeStr, paddingX, startY + 24);

                // 3. Gambar Alamat Full (dibungkus agar tidak terpotong)
                ...
                for (let i = 0; i < addressLines.length; i++) {
                    ...
                    curY += 18;
                    // Max 4 baris alamat supaya tidak out of bounds
                    if(i === 3) break; 
                }

                // Gambar Koordinat Lat Long
                if (currentLat !== null) {
