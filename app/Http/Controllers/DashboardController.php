...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        } elseif ($user->hasRole(['Director', 'Direktur', 'GM'])) {
    ...
    private function getDirectorData(Request $request): array
    {
        ...
        // Total unique employees attended today (excluding Director, GM, Admin, Superadmin, and Intern)
        $todayAttendancesCount = DailyAttendance::whereDate('date', $now->toDateString())
            ->whereHas('user.roles', function($q) {
                $q->whereNotIn('name', ['Director', 'Direktur', 'GM', 'Admin', 'Superadmin', 'Intern']);
            })
            ->distinct('user_id')
            ->count();
        
        // Total unique staff (excluding Director, GM, Admin, Superadmin, and Intern)
